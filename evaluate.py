...
def bilstm_eval(test_data,word2id, tag2id, crf=True, remove_O=False):
    ...
    bilstm_model = load_model("./ckpts/"+model_name+".pkl")


    print("推理{}模型中...".format(model_name))
    pred_tag_lists_temp = []
    total_len = len(test_word_lists)

    for i in range(total_len):
        #print([test_word_lists[i]])
        pred_tag_lists, word_lists = bilstm_model.inferrence(
            [test_word_lists[i]], word2id, tag2id)
        pred = ''.join('')
        for word in pred_tag_lists[0]:
            pred +=str(word)
            pred +=' '
        pred+='\n'
        print(pred)
